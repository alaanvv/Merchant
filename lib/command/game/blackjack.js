const { getCoins, resolveCoins, addCoins, removeCoins } = require('../../util')
...
class Blackjack {
  ...
  playerTurn(action) {
    ...
    const stand = () => { this.dealerTurn() }

    if (action === 'hit') hit()
    else if (action === 'stand') stand()
    else if (action === 'double-down' && this.canDoubleDown) { hit(); stand() }
  }
...
module.exports = {
  ...
  run: async (bot, message, args) => {
    ...
    if (!wager) return message.reply(`Invalid value`)
    ...
    async function onEnd(playerWin, finalMessage, tie = false) {
      ...
      if (playerWin) await addCoins(message.author.id, wager)
      else await removeCoins(message.author.id, wager)
