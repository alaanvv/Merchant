...
class Blackjack {
  constructor(bot, _endCallback, canDoubleDown) {
    ...
    this.endCallback = (...args) => { this.end = true; _endCallback(...args) } // (playerWin, message)
  }
  ...
  playerTurn(action) {
    ...
    if (action === 'hit') {
      ...
      if (this.calculateHand(this.dealerHand) === 21 && dealerHandValue === playerHandValue) this.endCallback(false, 'Tie. No one wins!', true)
  ...
  dealerTurn() {
    ...
    while (dealerHandValue < Math.max(this.bot.rules.BLACKJACK_HIT_STOP, playerHandValue)) {
    ...
    else if (dealerHandValue === 21 && dealerHandValue !== playerHandValue) this.endCallback(false, 'Dealer\'s blackjack. Player loose!')
    else if (dealerHandValue > playerHandValue) this.endCallback(false, 'Dealer is near to 21. Player loose!')
...
module.exports = {
  ...
  run: async (bot, message, args) => {
    ...
    collector.on('end', () => {
      ...
      if (!blackjack.end) blackjack.onEnd(false, 'You loose because of timeout')
    })
  }
}
